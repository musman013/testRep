...
import { IWorkspace, WorkspaceService, WorkspaceDetailsComponent, WorkspaceListComponent, WorkspaceNewComponent } from './index';

describe('WorkspaceListComponent', () => {
  let fixture:ComponentFixture<WorkspaceListComponent>;
  let component:WorkspaceListComponent;
  let el: HTMLElement;
  let constData:IWorkspace[] = [
    {   
      id: 1,
      name: 'name1',
    },
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(location.path()).toBe(`/workspace/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`);
    });
